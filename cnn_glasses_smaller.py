...
class Model:
  ...
  def __init__(self, X_train_, X_test_, y_train_, y_test_, filter_size=5, NUM_STEPS_ = 50000, MINIBATCH_SIZE_ = 25, learning_rate_ = 0.00001, size_hidden_layer_ = 64):
    ...
    self.conv2 = self.conv_layer(self.conv1_pool, shape=[filter_size, filter_size, 32, 64]) # the result here will be 56X56X64
    self.conv2_pool = self.max_pool_2x2(self.conv2) # the result will be 28X28X64

    # create a third layer
    self.conv3 = self.conv_layer(self.conv2_pool, shape=[filter_size, filter_size, 64, 128]) # the result here will be 28X28X128
    self.conv3_pool = self.max_pool_2x2(self.conv3) # the result will be 14X14X128

    # create a forth layer
    self.conv4 = self.conv_layer(self.conv3_pool, shape=[filter_size, filter_size, 128, 256]) # the result here will be 14X14X256
